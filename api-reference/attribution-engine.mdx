...
| | `datasource_created` |
| **Revenue** | `customer.subscription.created` |
...
<Warning>
Stripe's `customer.created` and `invoice.paid` webhooks are still stored internally for analytics but are no longer reported to ad platforms (paused - see [Reporting rules](#reporting-rules)). Only `customer.subscription.created` ("converted lead", fired once at first paid subscription) is reported, since renewals happen well outside any platform's attribution window and add no bidding signal.
</Warning>

To view events per ad platform, refer to the [ad platform event mapping tables](#ad-platforms).
...
<Steps>
  ...
  <Step title="Revenue">
    ...
    **Only `customer.subscription.created` (first paid subscription) is sent to ad platforms**, with the real transaction amount as conversion value - it's a one-off event that lands within the attribution window. `customer.created`/`invoice.paid` are stored for internal analytics only, no longer reported to Meta/Google (recurring renewals fall outside any attribution window and don't help Smart Bidding).
  </Step>
</Steps>
...
- **Reporting rules** (business logic): once an event is stored, we still decide *whether it counts as a reportable conversion* - once per visitor for leads, once per company for signups. This is what prevents inflated metrics, not idempotency.
  ...
  | **Activation events** | Always (when org has first-time owner) | Pixel enrichment/signals only (not campaign conversion goals). Reports to Google only, not Meta. |
  | **Revenue events** | `customer.subscription.created` only, once per first-time owner | `customer.created`/`invoice.paid` are paused (stored for analytics, not reported). `customer.subscription.created` reports to Google only, not Meta, with the real transaction amount as conversion value. |
...
<Tabs>
  <Tab title="Google Ads">
    ...
    **Event mapping:**
    ...
    | <code style={{whiteSpace: 'nowrap'}}>agent_created</code><br/><code style={{whiteSpace: 'nowrap'}}>datasource_created</code> | `Qualified lead` | Enhanced Conversions for Leads (Data Manager API) | Enrichment (Smart Bidding signal) |
    | `customer.subscription.created` | `Converted lead` | Enhanced Conversions for Leads (Data Manager API) | Enrichment (Smart Bidding signal), sent with real transaction value |
    
    <Info>
    `customer.created`/`invoice.paid` are **not reported** - paused, see [Reporting rules](#reporting-rules).
    </Info>
    ...
    **Attribution matching:**
    ...
    |--------|-----------|----------|-------------------|
    | **Click ID** (primary) | gclid, gbraid, wbraid → direct link to keyword/ad/campaign | User clicked Google ad, deterministic attribution | 90d click (matches Ads Manager conversion action settings) |
    | **Enhanced conversions** (secondary) | Hashed email/phone/name, looked up from the org owner's account when no click ID survives the window. Works WITHOUT gclid. | Recovers lost gclid (cookie deletion, cross-device, or activation/revenue events that happen well after the original click). Required for full coverage. | Same as above |
  ...
  <Tab title="Meta (Facebook/Instagram)">
    ...
    **Event mapping:**
    ...
    <Info>
    Activation events (<code>agent_created</code>, <code>datasource_created</code>) and <code>qualified_lead</code>/<code>customer.subscription.created</code> are **Google-only** - deliberately not sent to Meta. `customer.created`/`invoice.paid` are **not reported** at all - paused, see [Reporting rules](#reporting-rules).
    </Info>
...
- **Google:** 90-day click window (matches our conversion actions' configured click-through window in Ads Manager)
...
**Result:** Reports using `fbclid_new` (most recent Meta click) + `gclid` (most recent Google click within 90d).
...
<AccordionGroup>
  ...
  <Accordion title="What if someone clicks multiple ads?">
    ...
    **Why this approach:**
    ...
    - Platforms apply their own attribution windows (7d Meta, 90d Google)
  ...
  <Accordion title="What's the attribution window?">
    **Our implementation:** Last-click attribution with platform-specific windows
    ...
    - **Google:** 90-day click window (most recent click wins, matches our conversion actions' configured window in Ads Manager)
    ...
    **Platform defaults:**
    ...
    - Google: our conversion actions are configured for the max 90-day click window (configurable per conversion action, default 30d)
  </Accordion>
