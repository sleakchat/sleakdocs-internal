...
<Warning>
**`customer.created` and `invoice.paid` are no longer reported to ad platforms** (paused - see [Reporting rules](#reporting-rules)). Only `customer.subscription.created` ("converted lead", fired once at first paid subscription) is still reported, since renewals happen well outside any platform's attribution window and add no bidding signal.
</Warning>
...
- **Reporting rules** (business logic): once an event is stored, we still decide *whether it counts as a reportable conversion* - once per visitor for leads, once per company for signups. This is what prevents inflated metrics, not idempotency.
  ...
  | **Activation events** | Always (when org has first-time owner) | Pixel enrichment/signals only (not campaign conversion goals). `qualified_lead` reports to Google only, not Meta. |
...
<Tabs>
  <Tab title="Google Ads">
    ...
    **Event mapping:**
    ...
    | <code style={{whiteSpace: 'nowrap'}}>agent_created</code><br/><code style={{whiteSpace: 'nowrap'}}>datasource_created</code><br/><code style={{whiteSpace: 'nowrap'}}>widget_installed</code> | Custom event | Enhanced Conversions for Leads (Data Manager API) | Enrichment (Smart Bidding signal) |
    | `qualified_lead` | `Qualified lead` | Enhanced Conversions for Leads (Data Manager API) | Enrichment (Smart Bidding signal) |
  ...
  <Tab title="Meta (Facebook/Instagram)">
    ...
    **Event mapping:**
    ...
    <Info>
    `qualified_lead` and `customer.subscription.created` are **Google-only** - deliberately not sent to Meta. `customer.created`/`invoice.paid` are **not reported** at all - paused, see [Reporting rules](#reporting-rules).
    </Info>
