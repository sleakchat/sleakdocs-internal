...
| Event Type | Trigger | Reporting Rule |
|------------|---------|---------------------|
| **Lead** | Demo form, Calendly booking, contact form, gated content | Once per visitor (same browser). Different employees = separate leads. |
| **Signup** | Trial signup, account creation | Once per company. Only when owner creates their first workspace. Team members not counted. |
| **Activation** | First agent, datasource, widget install, qualified lead | Reported for enrichment only (not conversion goal). |
| **Revenue** | Stripe payment, subscription created/renewed | Reported for enrichment only (not conversion goal). Includes revenue amount. |
...
<Steps>
  ...
  <Step title="Lead submission">
    ...
    **What happens:**
    ...
    4. Checks: has this `anonymous_id` reported a lead before?
    5. If no: reports to Meta/Google with enriched data
    6. If yes: stores in DB only, no ad platform report
...
- **Reporting rules** (business logic): once an event is stored, we still decide *whether it counts as a reportable conversion* - once per visitor for leads, once per company for signups. This is what prevents inflated metrics, not idempotency.
  ...
  |------------|----------------|---------|
  | **Lead** | Once per visitor | Prevents duplicate leads from same browser (form resubmits, typo corrections) |
...
Visit this URL in your work browser: https://data.sleak.chat/mark-internal-browser

Your click IDs will be excluded from attribution. Re-visit if you clear browser data.
...
<Tabs>
  <Tab title="Google Ads">
    ...
    **Event mapping:**
    ...
    | <code style={{whiteSpace: 'nowrap'}}>checkout.session.completed</code><br/><code style={{whiteSpace: 'nowrap'}}>invoice.payment_succeeded</code><br/><code style={{whiteSpace: 'nowrap'}}>customer.subscription.*</code> | Custom event | Google Ads API<br/>+ Google gtag | Enrichment (Smart Bidding signal) |
  ...
  <Tab title="Meta (Facebook/Instagram)">
    ...
    **Event mapping:**
    ...
    | <code style={{whiteSpace: 'nowrap'}}>checkout.session.completed</code><br/><code style={{whiteSpace: 'nowrap'}}>invoice.payment_succeeded</code><br/><code style={{whiteSpace: 'nowrap'}}>customer.subscription.*</code> | `Purchase`<br/>`Subscribe` | Meta Conversions API (CAPI)<br/>+ Facebook Pixel | Enrichment (not conversion goal) |
...
```javascript Call Booked
// Calendly/call bookings
window.sleakAttribution.track('call_booked', {
  email: 'john@acme.com',
  first_name: 'John',
  last_name: 'Doe',
  phone: '+1234567890'
...
<Info>
**Signup events:** Pass `user_id` as the third parameter. Server looks up organization and member from user_id.

**Lead events:** No third parameter needed - leads don't require authentication.

Activation and revenue events are reported automatically on the backend and should never be sent through `track()`.
</Info>
...
| Endpoint | Purpose |
|----------|---------|
| `/api/events/leads` | Lead events (demo forms, Calendly) |
